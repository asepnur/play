...
func main() {
	...
	for _, v := range source {
		for _, w := range data {
			closeDistance := 1000
			if distance := Levenshtein(strings.ToLower(v), strings.ToLower(w)); distance <= 1 {
				if distance < closeDistance {
					closeDistance = distance
					match = append(match, v)
					log.Println("match: ", v, " - ", w)
				}
			}
		}
	}
